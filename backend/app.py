...
def init_worker():
    ...
    print(
        f"✅ Worker process {multiprocessing.current_process().pid} initialized MediaPipe"
    )
    ...
    def smooth_3d_angles(self, angles):
        ...
        for key, value in angles.items():
            if key in self.smoothed_3d_angles:
                smoothed[key] = (
                    self.alpha * value + (1 - self.alpha) * self.smoothed_3d_angles[key]
                )
            else:
                smoothed[key] = value
    ...
    def smooth_3d_coords(self, coords):
        ...
        for joint, coord in coords.items():
            if joint in self.smoothed_3d_coords:
                smoothed[joint] = {
                    "x": self.alpha * coord["x"]
                    + (1 - self.alpha) * self.smoothed_3d_coords[joint]["x"],
                    "y": self.alpha * coord["y"]
                    + (1 - self.alpha) * self.smoothed_3d_coords[joint]["y"],
                    "z": self.alpha * coord["z"]
                    + (1 - self.alpha) * self.smoothed_3d_coords[joint]["z"],
                }
            else:
        ...
        try:
            ...
            if pose_results.pose_landmarks:
                        ...
                        body_landmarks.append(
                            {
                                "name": name,
                                "x": round(
                                    lm.x * original_width, 1
                                ),  # Use ORIGINAL dimensions
                                "y": round(
                                    lm.y * original_height, 1
                                ),  # Use ORIGINAL dimensions
                                "confidence": round(lm.visibility * 100),
                                "visible": lm.visibility > 0.3,
                            }
                ...
                if use3D and pose_results.pose_world_landmarks:
                    ...
                    pose_3d_angles, pose_3d_coords = calculate_3d_angles_mediapipe(
                        world_landmarks
                    )
            ...
            if processed_frame_count % 30 == 0:
                print(
                    f"⚡ Backend [Frame {processed_frame_count}]: "
...
def estimate_pose_reference():
    ...
    try:
        ...
        result = reference_pool.apply(
            process_reference_frame_worker, (frame_bytes, width, height)
        )
...
def list_downloads():
    ...
    try:
        ...
        for file in downloads_dir.iterdir():
            print(
                f"[DEBUG] Found file: {file.name} (is_file: {file.is_file()}, suffix: {file.suffix})"
            )
...
def upload_video():
    """
    Upload a video file directly from user's computer
    ...
    try:
        ...
        allowed_extensions = {".mp4", ".webm", ".mkv", ".avi", ".mov", ".m4v"}
        ...
        while file_path.exists():
            filename = f"{original_stem}_{counter}{file_ext}"
            file_path = downloads_dir / filename
            counter += 1

        file.save(str(file_path))

        print(f"[DEBUG] Video uploaded: {file_path}")
    ...
    try:
        ...
        if range_header:
            ...
            response = send_from_directory(
                downloads_dir, filename, as_attachment=False, mimetype="video/mp4"
            )
            response.headers.add("Accept-Ranges", "bytes")
            return response
